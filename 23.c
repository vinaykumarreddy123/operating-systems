...
void firstFit(int process_id, int process_size) {
    int i;
    ...
    for (i = 0; i < num_partitions; i++) {
        if (partition_size[i] >= process_size) {
            ...
            printf("Process %d of size %d allocated to partition %d of size %d\n",
                   process_id, process_size, partitions[i], partition_size[i]);
            partition_size[i] -= process_size;
...
int main() {
    ...
    // Example process allocations using first fit
    firstFit(1, 120);
    displayMemoryStatus();
